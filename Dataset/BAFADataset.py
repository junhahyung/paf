...
class TFDataset():
    ...
    def preprocess(self, record):
        ...
        def get_random_size():
            size_x = tf.random.normal([1], 240, 20) # 240, 24
            size_x = tf.clip_by_value(size_x, 192, 288) #192, 240
            size_x = tf.cast(size_x, tf.int32)
            size_y = tf.random.normal([1], 240, 36) # 240, 24
        ...
        # heatmap
        rotated_bm = tfa.image.rotate(boundary, theta, 'BILINEAR')
        resized_bm = tf.image.resize(rotated_bm, size2d)
